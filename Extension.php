...
class Extension extends \Bolt\BaseExtension
{
    ...
    private function buildField($name, $field, $with = array(), $formname = 'null')
    {
        ...
        if (isset($this->config[$formname])) {
            ...
            $this->app['logger.system']->info("Attempting to set a form field without a form",
                array('event' => 'extensions'));
        ...
        foreach ($mappings as $src => $dst) {
            if (!empty($field[$src])) {
                ...
                switch (count($dstPath)) {
                    case 1:
                        $options[$dstPath[0]] = $value;
                        break;
                    case 2:
                        $options[$dstPath[0]][$dstPath[1]] = $value;
                        break;
                    default:
        ...
        if ($options['required'] && empty($field['fieldconstraints'])) {
            $options['constraints'][] = new Assert\NotBlank();
        }

        if ($field['type'] === 'email' && empty($field['fieldconstraints'])) {
            $options['constraints'][] = new Assert\Email();
        }

        if ($field['fieldconstraints'] && is_array($field['fieldconstraints'])) {
            foreach ($field['fieldconstraints'] as $key => $value) {
                switch ($key) {
                    case 'notblank':
                        ...
                        if (isset($value['message'])) {
                            $constraint_options['message'] = $value['message'];
                        }

                        $options['constraints'][] = new Assert\NotBlank($constraint_options);
                        break;
                    case 'length':
                        ...
                        if (isset($value['min'])) {
                            $constraint_options['min'] = $value['min'];
                            if (isset($value['minMessage'])) {
                                $constraint_options['minMessage'] = $value['minMessage'];
                            }
                        }
                        if (isset($value['max'])) {
                            $constraint_options['max'] = $value['max'];
                            if (isset($value['maxMessage'])) {
                                $constraint_options['maxMessage'] = $value['maxMessage'];
                            }
                        }

                        $options['constraints'][] = new Assert\Length($constraint_options);
                        break;
                    case 'email':
                        $constraint_options = array();
                        ...
                        $options['constraints'][] = new Assert\Email($constraint_options);
    ...
    public function simpleForm($formname = "", $with = array())
    {
        ...
        $form = $this->app['form.factory']->createNamedBuilder($formname, 'form', null,
            array('csrf_protection' => $this->config['csrf']));
        ...
        $formhtml = $this->app['render']->render($formconfig['template'], array(
            ...
            "button_class" => $formconfig['button_class']
        ));
    ...
    private function processForm($formconfig, $form, $formname)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            if ($formconfig['fields'][$key]['type'] == 'choice' && !empty($formconfig['fields'][$key]['choices'])) {
                ...
                if (isset($field['use_as']) && in_array($field['use_as'],
                        array('from_email', 'to_email', 'cc_email', 'bcc_email'))
                ) {
        ...
        if (!empty($formconfig['insert_into_table'])) {
            try {
                ...
                $this->app['logger.system']->info("SimpleForms could not insert data into table: {$errortxt} - check if the table exists",
                    array('event' => 'extensions'));
        ...
        if (!empty($formconfig['recipient_email'])) {
            ...
            $this->app['logger.system']->info('Set Recipient for ' . $formname . ' to ' . $formconfig['recipient_email'],
                array('event' => 'extensions'));
        }
        ...
        if (!empty($formconfig['from_email'])) {
            ...
            $this->app['logger.system']->info('Set Sender for ' . $formname . ' to ' . $formconfig['from_email'],
                array('event' => 'extensions'));
        }
        ...
        if ($formconfig['testmode'] == true) {
            ...
            if (!empty($formconfig['recipient_cc_email']) && $formconfig['recipient_email'] != $formconfig['recipient_cc_email']) {
                $this->app['logger.system']->info('Did not set Cc for ' . $formname . ' to ' . $formconfig['recipient_cc_email'] . ' (in testmode)',
                    array('event' => 'extensions'));
            }
            if (!empty($formconfig['recipient_bcc_email']) && $formconfig['recipient_email'] != $formconfig['recipient_bcc_email']) {
                $this->app['logger.system']->info('Did not set Bcc for ' . $formname . ' to ' . $formconfig['recipient_bcc_email'] . ' (in testmode)',
                    array('event' => 'extensions'));
            }
        } else {
            ...
            if (!empty($formconfig['recipient_cc_email']) && $formconfig['recipient_email'] != $formconfig['recipient_cc_email']) {
                ...
                $this->app['logger.system']->info('Added Cc for ' . $formname . ' to ' . $formconfig['recipient_cc_email'],
                    array('event' => 'extensions'));
            }
            if (!empty($formconfig['recipient_bcc_email']) && $formconfig['recipient_email'] != $formconfig['recipient_bcc_email']) {
                $message->setBcc($formconfig['recipient_bcc_email']);
                $this->app['logger.system']->info('Added Bcc for ' . $formname . ' to ' . $formconfig['recipient_bcc_email'],
                    array('event' => 'extensions'));
            }
        ...
        // log the attempt
        $this->app['logger.system']->info('Sending message ' . $formname . ' from ' . $formconfig['from_email'] . ' to ' . $formconfig['recipient_email'],
            array('event' => 'extensions'));
        ...
        if ($res) {
            if ($formconfig['testmode']) {
                $this->app['logger.system']->info('Sent email from ' . $formname . ' to ' . $formconfig['testmode_recipient'] . ' (in testmode) - ' . $formconfig['recipient_name'],
                    array('event' => 'extensions'));
            } else {
                $this->app['logger.system']->info('Sent email from ' . $formname . ' to ' . $formconfig['recipient_email'] . ' - ' . $formconfig['recipient_name'],
                    array('event' => 'extensions'));
            }
        }
    ...
    private function getSequence($formconfig, $form, $formname, $column)
    {
        ...
        if (!empty($formconfig['insert_into_table'])) {
            try {
                ...
                // Oops. User will get a warning on the dashboard about tables that need to be repaired.
                $this->app['logger.system']->info("SimpleForms could not fetch next sequence number from table" . $formconfig['insert_into_table'] . ' - check if the table exists.',
                    array('event' => 'extensions'));
